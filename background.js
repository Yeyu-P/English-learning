...
async function captureAndAnalyze() {
  ...
  // Insert pending record immediately so panel shows spinner
  ...
  // Open side panel so user sees progress
  try {
  ...
  const { apiKey, model = 'claude-sonnet-4-6' } = await chrome.storage.local.get(['apiKey', 'model']);
  ...
  if (!apiKey) {
    ...
    try {
      analysis = await analyzeWithClaude(screenshotUrl, apiKey, model);
    } catch (err) {
